...
//**********************************************
//* Chargement CRC отдельных ячеек 
//**********************************************
uint32_t nvexplorer::load_item_crc(int idx) {
  
uint32_t crc;

memcpy(&crc,pdata+itemoff_idx(idx)+itemlist[idx].len,4);
return crc;
}
...
uint32_t crc;
crc=calc_crc32(pdata+itemoff_idx(idx),itemlist[idx].len);
...
if (calc_item_crc(idx) != load_item_crc(idx)) return false;
return true;
}
